...
/**
 * The full pet-creation field set (category → breed → core → dynamic → images).
 * Shared by the standalone "Yeni pet" page and the inline new-pet flow in the
 * listing form. `descriptionName` lets the caller avoid a field-name clash with
 * its own `description` (the listing form passes `petDescription`).
 */
export function PetFieldset({
  categories,
        ...
        <>
          ...
          <section className="space-y-3">
            ...
            {category.freeTextBreed ? (
              <input name="breedFreeText" placeholder="Növü yazın" className={inputClass} />
            ) : (
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-1">
              <label htmlFor={descriptionName} className={labelClass}>
                Qısa təsvir / xasiyyət
              </label>
              <textarea id={descriptionName} name={descriptionName} rows={3} className={inputClass} />
            </div>
